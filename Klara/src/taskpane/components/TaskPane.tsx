...
function LoginOverlay({ onLogin }: { onLogin: (token: string, user: any) => void }) {
  ...
  const handleEmailLogin = async () => {
    ...
    try {
      ...
      const { setTokens: st } = await import('../api/client');
      const resp = await api.login({ username: email, password });
      st(resp.access_token, resp.refresh_token);
...
export function TaskPane() {
  ...
  const loadFindings = useCallback(async () => {
    if (!token) return;
    try {
      const data = await qcApi.getFindings('current');
      setFindings(data);
    } catch {
      setFindings([]);
    }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', position: 'relative' }}>
      ...
      <div className="klara-tabs">
        ...
        >
          Suggestions
          {findings.length > 0 && <span className="klara-badge">{findings.length}</span>}
        </button>
        <button
