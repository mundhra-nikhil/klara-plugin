...
export const qcApi = {
  getFindings: async (documentId: string): Promise<QCFinding[]> => {
    ...
    return items.map((item: any) => {
      ...
      return {
        ...
        replacement_text:
          item.replacement_text ?? loc.replacement_text ?? item.replacement_text ?? undefined,
  ...
  getReviews: async (documentId?: string): Promise<QCReview[]> => {
    const { data } = await apiClient.get("/qc/reviews", {
    ...
    if (!Array.isArray(result)) {
      throw new Error(
        `Unexpected API response structure: expected an array of reviews, got ${typeof body}`
      );
    }
    return result;
  },
